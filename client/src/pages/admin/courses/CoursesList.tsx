...
export default function CoursesList() {
  ...
  const fetchCourses = async () => {
    setLoading(true);
    try {
      const response = await adminCoursesApi.getAll({
        page,
        search: searchTerm,
        level: levelFilter
      });
  ...
  const handleDeleteToggle = async (course: Course) => {
    ...
    try {
      ...
      alert("Error al actualizar estado del curso");
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm flex flex-col md:flex-row justify-between items-center gap-4">
        ...
        <Button 
          variant="primary" 
          onClick={() => navigate('/admin/courses/new')} // Crearemos esto luego
          icon={<Plus size={18} />}
        >
          Nuevo Curso
        </Button>
      ...
      {/* Grid de Cursos (Cards en vez de Tabla para mejor visualización) */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {loading ? <p>Cargando...</p> : courses.map((course) => (
