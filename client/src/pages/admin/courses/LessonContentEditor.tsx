...
export default function LessonContentEditor() {
  ...
  const handleSaveTheory = async () => {
    ...
    try {
      ...
      alert("Teoría guardada correctamente");
    } catch (error) {
      alert("Error al guardar teoría");
    } finally {
              ...
              {exercises.map((ex, index) => (
                <div key={ex.id} className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm flex justify-between items-start gap-4">
                   <div className="flex gap-4">
                      <div className={`w-8 h-8 flex items-center justify-center rounded-lg font-bold text-xs shrink-0 ${
