...
export const InventoryTab = ({ userId }: { userId: number }) => {
    ...
    const handleGiveItem = async () => {
        ...
        try {
            ...
            toast.success("Ítem entregado exitosamente");
            setIsGiftMode(false);
            fetchInventory();
        } catch (e) { toast.error("Error al entregar ítem"); }
    };

    const handleRevokeItem = async (itemId: number) => {
        if (!confirm("¿Eliminar ítem del usuario?")) return;
        try {
    ...
    const handleGemUpdate = async () => {
        ...
        try {
            ...
            toast.error("Error al ajustar gemas");
        }
    }
