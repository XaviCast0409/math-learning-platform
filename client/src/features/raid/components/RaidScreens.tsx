...
// 👇 1. DEFINIMOS EL TIPO DE DATO PARA EL MVP
interface MvpData {
  username: string;
  totalDamage: number;
}
...
interface VictoryProps {
  ...
  rewards: { xp: number; gems: number } | null;
}

export const RaidVictoryScreen = ({ mvpData, rewards }: VictoryProps) => {
  const navigate = useNavigate();

  // Valores por defecto (fallback) si rewards es null
  const xp = rewards?.xp || 500;
  const gems = rewards?.gems || 50;
      ...
      <Card className="max-w-sm w-full bg-slate-800/90 border-slate-700 backdrop-blur-md p-6 z-10">
        <div className="space-y-4">
          <div className="flex justify-between items-center border-b border-slate-700 pb-3">
            ...
            {/* 👇 USAMOS LA VARIABLE {xp} EN LUGAR DE +500 FIJO */}
            <span className="text-yellow-400 font-black text-xl">+{xp}</span>
          </div>
          <div className="flex justify-between items-center pb-3">
            <span className="text-slate-400 font-bold uppercase text-xs tracking-wider">XaviCoins</span>
            {/* 👇 USAMOS LA VARIABLE {gems} EN LUGAR DE +50 FIJO */}
            <span className="text-blue-400 font-black text-xl">+{gems}</span>
          </div>
